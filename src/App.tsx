...
const API_KEY  = "AIzaSyD-qMeKXarXTsP37UNjXSoSIx0KlTfOsBM";
const BASE_URL = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}`;

const sheetsRead = async (range: string): Promise<string[][]> => {
  const res = await fetch(`${BASE_URL}/values/${range}?key=${API_KEY}`);
  if (!res.ok) return [];
  const data = await res.json();
  return (data.values as string[][]) || [];
};
const sheetsWrite = async (range: string, values: any[][]): Promise<void> => {
  await fetch(`${BASE_URL}/values/${range}?valueInputOption=RAW&key=${API_KEY}`, {
    method: "PUT", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ values }),
  });
};
const sheetsAppend = async (range: string, values: any[][]): Promise<void> => {
  await fetch(`${BASE_URL}/values/${range}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS&key=${API_KEY}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ values }),
  });
};
const sheetsClear = async (range: string): Promise<void> => {
  await fetch(`${BASE_URL}/values/${range}:clear?key=${API_KEY}`, { method: "POST" });
};

const usd = (n: number | string): string =>
  new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(Math.round(Number(n) || 0));
const pct = (a: number, b: number): number => (b > 0 ? Math.round((a / b) * 100) : 0);
const uid = (): string => Date.now() + "_" + Math.random().toString(36).slice(2, 7);

const CAT_EMOJI: Record<string, string> = {
  Housing:"🏠",Transport:"🚗",Utilities:"💡",Insurance:"🛡️",Subscriptions:"📱",
  Debt:"💳",Groceries:"🛒",Dining:"🍕",Gas:"⛽",Health:"💊",Shopping:"🛍️",
  Entertainment:"🎬",Kids:"👶","Personal Care":"💅",Travel:"✈️",Other:"📦",
};
...
const autocat = (name: string): string => {
...
type Bill     = { id:string; name:string; amt:number; cat:string; freq:string };
type Expense  = { id:string; name:string; amt:number; cat:string };
type OtherInc = { id:string; name:string; amt:number; freq:string };
type Card     = { id:string; name:string; limit:number; bal:number; apr:number; due:string; min:number };
type FunAcc   = { bal:number; mo:number };
type InvState = { my401k:number;sp401k:number;my457b:number;sp457b:number;myC:number;spC:number;match:number;roth:number;s529:number;utma:number };
type HysaState= { efBal:number;efC:number;vacBal:number;vacGoal:number;vacC:number };
type RetireS  = { myAge:number;retireAt:number;rate:number };
type Scores   = { me:number;sp:number };
type Fw       = { n:number;w:number;s:number };

interface FProps {
  lbl?: string; ph?: string; type?: string;
  def?: string | number; save?: (v:string)=>void;
  pre?: boolean; full?: boolean; sx?: React.CSSProperties;
}

function F({ lbl, ph, type="text", def="", save, pre, full, sx }: FProps) {
  const ref = useRef<HTMLInputElement>(null);
  useEffect(() => { if (ref.current) ref.current.value = def !== undefined ? String(def) : ""; }, []);
  const base: React.CSSProperties = {
    width:"100%", background:"#1e2f45", border:"1px solid rgba(255,255,255,0.09)",
    borderRadius:10, padding:"11px 13px", fontSize:15, color:"#e8edf4",
    fontFamily:"inherit", outline:"none", boxSizing:"border-box", WebkitAppearance:"none" as any,
  };
  return (
    <div style={{ marginBottom: full ? 12 : 0 }}>
      {lbl && <div style={{ fontSize:11,color:"#8fa3be",textTransform:"uppercase",letterSpacing:0.6,marginBottom:5 }}>{lbl}</div>}
      <div style={{ position:"relative" }}>
        ...
        <input ref={ref} type={type} inputMode={type==="number"?"decimal":"text"} placeholder={ph}
          ...
          style={{ ...base, ...(pre ? { paddingLeft:26 } : {}), ...(sx||{}) }} />
      </div>
    </div>
  );
}
...
const Chips = ({ opts, val, set }: { opts:string[]; val:string; set:(v:string)=>void }) => (
  <div style={{ display:"flex",flexWrap:"wrap",gap:5,marginTop:4 }}>
    {opts.map(o => (
      <span key={o} onClick={()=>set(o)} style={{
        padding:"5px 11px",borderRadius:20,fontSize:12,fontWeight:500,cursor:"pointer",
        border:`1px solid ${val===o?"#00c9a7":"rgba(255,255,255,0.08)"}`,
        background:val===o?"rgba(0,201,167,.15)":"#1e2f45",
        color:val===o?"#00c9a7":"#8fa3be",userSelect:"none",
      }}>{CAT_EMOJI[o]||""} {o}</span>
    ))}
  </div>
);
  ...
  <div style={{ display:"flex",background:"#162236",borderRadius:10,padding:3,gap:3,marginBottom:14 }}>
    {opts.map(([id,lbl]) => (
      <button key={id} onClick={()=>set(id)} style={{
        ...
        cursor:"pointer",background:val===id?"#263852":"transparent",
        color:val===id?"#e8edf4":"#8fa3be",fontFamily:"inherit",
      }}>{lbl}</button>
    ))}
  </div>
);

const Card = ({ children, sx }: { children:React.ReactNode; sx?:React.CSSProperties }) => (
  <div style={{ background:"#162236",borderRadius:18,padding:"14px 16px",marginBottom:12,border:"1px solid rgba(255,255,255,0.07)",...(sx||{}) }}>{children}</div>
);
const Barz = ({ p, col }: { p:number; col:string }) => (
  <div style={{ height:7,background:"#1e2f45",borderRadius:4,overflow:"hidden",marginTop:5 }}>
    <div style={{ height:"100%",width:`${Math.min(100,p)}%`,background:col,borderRadius:4,transition:"width .5s" }}/>
  </div>
);
const X = ({ go }: { go:()=>void }) => (
  <button onClick={go} style={{ background:"none",border:"none",color:"#4a6080",fontSize:20,cursor:"pointer",padding:"0 4px",lineHeight:1 }}>×</button>
);
const ACard = ({ type, text }: { type:string; text:string }) => {
  const m: Record<string,string[]> = {
    tip:["rgba(0,201,167,.1)","rgba(0,201,167,.3)","#00c9a7","✦ Tip"],
    warning:["rgba(245,166,35,.1)","rgba(245,166,35,.3)","#f5a623","⚠ Watch"],
    alert:["rgba(231,76,60,.1)","rgba(231,76,60,.3)","#e74c3c","⚡ Action"],
  };
  const [bg,bd,ac,bj] = m[type]||m["tip"];
  return (
    <div style={{ background:bg,border:`1px solid ${bd}`,borderRadius:14,padding:14,marginBottom:10 }}>
      <div style={{ display:"inline-block",fontSize:10,fontWeight:700,padding:"3px 9px",borderRadius:10,marginBottom:7,background:bd,color:ac,textTransform:"uppercase",letterSpacing:0.5 }}>{bj}</div>
      <div style={{ fontSize:13,color:"#e8edf4",lineHeight:1.55 }}>{text}</div>
    </div>
  );
};

const TABS = [
  {id:"dash",icon:"📊",lbl:"Overview"},{id:"inc",icon:"💰",lbl:"Income"},
  {id:"bills",icon:"📋",lbl:"Bills"},{id:"exp",icon:"🛒",lbl:"Spend"},
  {id:"cc",icon:"💳",lbl:"Credit"},{id:"inv",icon:"📈",lbl:"Invest"},
  {id:"goals",icon:"🎯",lbl:"Goals"},
];
const TITLES: Record<string,[string,string]> = {
  dash:["Financial Overview", new Date().toLocaleString("default",{month:"long",year:"numeric"})],
  inc:["Income","Household earnings"],bills:["Bills","Fixed obligations"],
  exp:["Expenses","Variable spending"],cc:["Credit Cards","Track balances & limits"],
  inv:["Investments","401k · 457b · 529 · UTMA"],goals:["Goals & Savings","Funds, scores & fun money"],
};
...
export default function App() {
  ...
  const [otherInc, setOtherInc] = useState<OtherInc[]>([]);
  ...
  const [inv,      setInv]      = useState<InvState>({ my401k:0,sp401k:0,my457b:0,sp457b:0,myC:0,spC:0,match:50,roth:0,s529:0,utma:0 });
  const [retire,   setRetire]   = useState<RetireS>({ myAge:0,retireAt:65,rate:7 });
  const [hysa,     setHysa]     = useState<HysaState>({ efBal:0,efC:0,vacBal:0,vacGoal:0,vacC:0 });
  const [myFun,    setMyFun]    = useState<FunAcc>({ bal:0,mo:0 });
  const [spFun,    setSpFun]    = useState<FunAcc>({ bal:0,mo:0 });
  const [scores,   setScores]   = useState<Scores>({ me:0,sp:0 });
  const [fw,       setFw]       = useState<Fw>({ n:50,w:30,s:20 });
...
useEffect(() => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (settings.length && settings[0].length) {
        const r = settings[0];
        const g = (i:number, d=0) => parseFloat(r[i])||d;
        setTakehome(g(0));
      ...
      setBills(billRows.map((r:string[])=>({ id:r[0],name:r[1],amt:parseFloat(r[2])||0,cat:r[3],freq:r[4] })));
      const expRows = await sheetsRead("Expenses!A2:D500");
      setExpenses(expRows.map((r:string[])=>({ id:r[0],name:r[1],amt:parseFloat(r[2])||0,cat:r[3] })));
      const incRows = await sheetsRead("OtherIncome!A2:D50");
      setOtherInc(incRows.map((r:string[])=>({ id:r[0],name:r[1],amt:parseFloat(r[2])||0,freq:r[3] })));
      const cardRows = await sheetsRead("Cards!A2:G50");
      setCards(cardRows.map((r:string[])=>({ id:r[0],name:r[1],limit:parseFloat(r[2])||0,bal:parseFloat(r[3])||0,apr:parseFloat(r[4])||20,due:r[5]||"",min:parseFloat(r[6])||0 })));
  ...
  const saveSettings = useCallback(async (overrides:any={}) => {
    setSyncing(true);
    const th = overrides.takehome ?? takehome;
    const f  = overrides.fw      ?? fw;
    const rt = overrides.retire  ?? retire;
    const iv = overrides.inv     ?? inv;
    const hy = overrides.hysa    ?? hysa;
    const mf = overrides.myFun   ?? myFun;
    const sf = overrides.spFun   ?? spFun;
    const sc = overrides.scores  ?? scores;
    await sheetsWrite("Settings!A1:AB1",[[
      th,f.n,f.w,f.s,rt.myAge,rt.retireAt,rt.rate,
      iv.my401k,iv.sp401k,iv.my457b,iv.sp457b,iv.myC,iv.spC,iv.match,iv.roth,iv.s529,iv.utma,
      hy.efBal,hy.efC,hy.vacBal,hy.vacGoal,hy.vacC,mf.bal,mf.mo,sf.bal,sf.mo,sc.me,sc.sp,
    ]]);
    setLastSync(new Date()); setSyncing(false);
  }, [takehome,fw,retire,inv,hysa,myFun,spFun,scores]);

  const saveBills = useCallback(async (nb:Bill[]) => {
    setSyncing(true);
    await sheetsClear("Bills!A2:E200");
    if (nb.length) await sheetsAppend("Bills!A2:E200", nb.map(b=>[b.id,b.name,b.amt,b.cat,b.freq]));
    setLastSync(new Date()); setSyncing(false);
  }, []);
  const saveExpenses = useCallback(async (ne:Expense[]) => {
    setSyncing(true);
    await sheetsClear("Expenses!A2:D500");
    if (ne.length) await sheetsAppend("Expenses!A2:D500", ne.map(e=>[e.id,e.name,e.amt,e.cat]));
    setLastSync(new Date()); setSyncing(false);
  }, []);
  const saveOtherInc = useCallback(async (ni:OtherInc[]) => {
    setSyncing(true);
    await sheetsClear("OtherIncome!A2:D50");
    if (ni.length) await sheetsAppend("OtherIncome!A2:D50", ni.map(o=>[o.id,o.name,o.amt,o.freq]));
    setLastSync(new Date()); setSyncing(false);
  }, []);
  const saveCards = useCallback(async (nc:Card[]) => {
    setSyncing(true);
    await sheetsClear("Cards!A2:G50");
    if (nc.length) await sheetsAppend("Cards!A2:G50", nc.map(c=>[c.id,c.name,c.limit,c.bal,c.apr,c.due,c.min]));
    setLastSync(new Date()); setSyncing(false);
  }, []);

  const scr: React.CSSProperties = { padding:"0 16px 110px" };
  const R2:  React.CSSProperties = { display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12 };
  const R3:  React.CSSProperties = { display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12 };
  const ST:  React.CSSProperties = { fontSize:15,fontWeight:700,color:"#e8edf4",marginBottom:10,marginTop:6 };
  const ML:  React.CSSProperties = { fontSize:10,color:"#8fa3be",textTransform:"uppercase",letterSpacing:0.5 };
  const ER:  React.CSSProperties = { display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.06)" };
  const EI   = (bg:string): React.CSSProperties => ({ width:36,height:36,borderRadius:10,background:bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0 });
  const MC:  React.CSSProperties = { background:"#162236",borderRadius:16,padding:"14px 13px",border:"1px solid rgba(255,255,255,0.07)" };
  const MV   = (c:string): React.CSSProperties => ({ fontSize:22,fontWeight:800,color:c,marginTop:4 });
  const BTN: React.CSSProperties = { width:"100%",padding:13,background:"#00c9a7",color:"#0b1624",border:"none",borderRadius:12,fontSize:15,fontWeight:700,fontFamily:"inherit",cursor:"pointer" };
  const scColor = (h:number) => h>=740?"#2ecc71":h>=670?"#f5a623":"#e74c3c";
  const scLabel = (h:number) => h>=740?"Excellent":h>=670?"Good":h>=580?"Fair":"Needs Work";
  ...
  const Dash = () => {
    ...
    const ads:Array<{t:string;x:string}>=[];
    if(!inc){ads.push({t:"tip",x:"Start by entering your combined take-home pay in the Income tab."});}
      ...
      const dc=cards.filter(c=>c.due);
      if(dc.length)ads.push({t:"tip",x:`Payment reminders: ${dc.map(c=>`${c.name} due day ${c.due}`).join(" · ")}. Goal: pay every card to $0 each month.`});
    }
    const bars=[
      {lbl:"Needs / Bills",used:bil,target:inc*fw.n/100,flip:false},
      {lbl:"Wants / Spending",used:exp,target:inc*fw.w/100,flip:false},
    ...
    return(
      <div style={scr}>
        <div style={{display:"flex",justifyContent:"center",margin:"8px 0 16px",position:"relative"}}>
          <svg width="140" height="140" viewBox="0 0 140 140">
            ...
            <circle cx="70" cy="70" r={r} fill="none" stroke={col} strokeWidth="12" strokeLinecap="round"
              strokeDasharray={circ} strokeDashoffset={circ*(1-health/100)} transform="rotate(-90 70 70)"/>
            <circle cx="70" cy="70" r="40" fill="none" stroke="#263852" strokeWidth="9"/>
            <circle cx="70" cy="70" r="40" fill="none" stroke="#a78bfa" strokeWidth="9" strokeLinecap="round"
              strokeDasharray={2*Math.PI*40} strokeDashoffset={2*Math.PI*40*(1-savPct/100)} transform="rotate(-90 70 70)"/>
          </svg>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          {([["Joint Income",usd(inc),"#2ecc71","/month"],["Bills",usd(bil),"#e74c3c",pct(bil,inc)+"% of income"],["Discretionary",usd(exp),"#f5a623",pct(exp,inc)+"% of income"],["Net Surplus",usd(surplus),surplus>=0?"#00c9a7":"#e74c3c",surplus>=0?"available":"over budget"]] as [string,string,string,string][]).map(([l,v,c,s])=>(
            <div key={l} style={MC}><div style={ML}>{l}</div><div style={MV(c)}>{v}</div><div style={{fontSize:11,color:"#8fa3be",marginTop:3}}>{s}</div></div>
          ))}
        </div>
        {cardBal>0&&<Card><div style={{display:"flex",justifyContent:"space-between"}}><div><div style={ML}>Total Card Debt</div><div style={MV("#e74c3c")}>{usd(cardBal)}</div></div><div style={{textAlign:"right"}}><div style={ML}>Min. Payments</div><div style={MV("#f5a623")}>{usd(cardMin)}</div></div></div></Card>}
        ...
        <div style={{textAlign:"center",marginTop:16}}>
          ...
          {syncErr&&<div style={{fontSize:11,color:"#e74c3c",marginTop:4}}>⚠ Could not connect to Google Sheets. Check your API key.</div>}
        </div>
      </div>
    );
  };
  ...
  const Inc = () => {
    ...
    return(
      <div style={scr}>
        <Card>
          <div style={{fontSize:13,color:"#8fa3be",marginBottom:10}}>💑 Joint checking — combined household income</div>
          <F lbl="Combined Monthly Take-Home (after tax)" ph="10,500" type="number" pre
            def={String(takehome||"")} save={async (v:string)=>{const val=parseFloat(v)||0;setTakehome(val);await saveSettings({takehome:val});}} full/>
          <div style={{fontSize:11,color:"#4a6080",marginTop:4}}>Your actual net deposit — both salaries after 401(k) deductions.</div>
        </Card>
        ...
        <Card>
          <div style={R2}>
            <F lbl="Description" ph="Freelance, bonus…" save={(v:string)=>{nr.current=v;}}/>
            <F lbl="Monthly $" type="number" pre ph="500" save={(v:string)=>{ar.current=v;}}/>
          </div>
          <button style={BTN} onClick={async()=>{
            if(!nr.current||!ar.current)return;
            const n:OtherInc[]=[...otherInc,{id:uid(),name:nr.current,amt:parseFloat(ar.current)||0,freq:"monthly"}];
  ...
  const Bills2 = () => {
    ...
    return(
      <div style={scr}>
        ...
        <Card>
          <div style={R2}>
            <F lbl="Bill Name" ph="Mortgage, Netflix…" save={(v:string)=>{nr.current=v;}}/>
            <F lbl="Amount" type="number" pre ph="1,800" save={(v:string)=>{ar.current=v;}}/>
          </div>
          <div style={{marginBottom:12}}><div style={{fontSize:11,color:"#8fa3be",textTransform:"uppercase",letterSpacing:0.6,marginBottom:5}}>Category</div><Chips opts={BILL_CATS} val={billCat} set={setBillCat}/></div>
          <button style={BTN} onClick={async()=>{
            if(!nr.current||!ar.current)return;
            const n:Bill[]=[...bills,{id:uid(),name:nr.current,amt:parseFloat(ar.current)||0,cat:billCat,freq:billFreq}];
  ...
  const Exp = () => {
    ...
    return(
      <div style={scr}>
        <div onClick={()=>(document.getElementById("csvIn") as HTMLInputElement)?.click()} style={{border:"2px dashed rgba(0,201,167,.35)",borderRadius:16,padding:"20px 16px",textAlign:"center",marginBottom:14,cursor:"pointer",position:"relative"}}>
        ...
        <Card>
          <div style={R2}>
            <F lbl="Description" ph="Groceries, dining…" save={(v:string)=>{nr.current=v;}}/>
            <F lbl="Amount" type="number" pre ph="120" save={(v:string)=>{ar.current=v;}}/>
          </div>
          <div style={{marginBottom:12}}><div style={{fontSize:11,color:"#8fa3be",textTransform:"uppercase",letterSpacing:0.6,marginBottom:5}}>Category</div><Chips opts={EXP_CATS} val={expCat} set={setExpCat}/></div>
          <button style={BTN} onClick={async()=>{
            if(!nr.current||!ar.current)return;
            const n:Expense[]=[...expenses,{id:uid(),name:nr.current,amt:parseFloat(ar.current)||0,cat:expCat}];
  ...
  const CC = () => {
    ...
    return(
      <div style={scr}>
        <Card><div style={{fontSize:13,color:"#8fa3be",lineHeight:1.55}}>🎯 <strong style={{color:"#e8edf4"}}>Goal:</strong> Pay every card to $0 each month. Keep utilization under 30% to protect your credit score.</div></Card>
        <Card>
          <div style={R2}>
            <F lbl="Card Name" ph="Chase Sapphire…" save={(v:string)=>{nr.current=v;}}/>
            <F lbl="Credit Limit" type="number" pre ph="10,000" save={(v:string)=>{lr.current=v;}}/>
          </div>
          <div style={R2}>
            <F lbl="Current Balance" type="number" pre ph="0" save={(v:string)=>{br.current=v;}}/>
            <F lbl="APR %" type="number" ph="20.99" save={(v:string)=>{ar.current=v;}}/>
          </div>
          <F lbl="Payment Due (day of month)" type="number" ph="25" save={(v:string)=>{dr.current=v;}} full/>
          <button style={BTN} onClick={async()=>{
            if(!nr.current||!lr.current)return;
            const lim=parseFloat(lr.current)||0,bal=parseFloat(br.current)||0;
            const n:Card[]=[...cards,{id:uid(),name:nr.current,limit:lim,bal,apr:parseFloat(ar.current)||20,due:dr.current,min:Math.max(25,bal*0.02)}];
            setCards(n);await saveCards(n);nr.current="";lr.current="";br.current="";ar.current="";dr.current="";
          }}>+ Add Card</button>
        </Card>
        {cards.map(c=>{
          ...
          return(
            <Card key={c.id}>
              ...
              <div style={R3}>
                {([["Balance",usd(c.bal),col],["Available",usd(avail),"#2ecc71"],["Limit",usd(c.limit),"#8fa3be"]] as [string,string,string][]).map(([l,v,cl])=>(
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:"#8fa3be"}}>Utilization</span><span style={{fontSize:12,fontWeight:700,color:col}}>{util}%</span></div>
              <Barz p={util} col={col}/>
              {c.bal>0&&<div style={{marginTop:10,padding:"8px 10px",background:"rgba(0,201,167,.08)",borderRadius:8,fontSize:12,color:"#8fa3be"}}>
                ...
                {c.apr>0&&payM>1&&<span> · At min. rate: ~{payM} months</span>}
                <div style={{marginTop:4,color:"#00c9a7",fontWeight:600}}>✓ Pay full {usd(c.bal)} by month-end → saves {usd(c.bal*c.apr/100/12)} interest</div>
              </div>}
              {util>30&&<div style={{marginTop:8,padding:"7px 10px",background:"rgba(231,76,60,.08)",borderRadius:8,fontSize:12,color:"#e74c3c"}}>⚠ Above 30% — pay down to {usd(c.limit*0.3)} for score boost</div>}
            </Card>
          );
        })}
  ...
  const Inv = () => {
    ...
    const upd=async(field:keyof InvState,val:string)=>{const ni={...inv,[field]:parseFloat(val)||0};setInv(ni);await saveSettings({inv:ni});};
    const upR=async(field:keyof RetireS,val:string)=>{const nr={...retire,[field]:parseFloat(val)||0};setRetire(nr);await saveSettings({retire:nr});};
    return(
      <div style={scr}>
        ...
        <Card>
          <div style={R2}>
            <F lbl="Your 401(k) Balance" type="number" pre ph="50,000" def={String(inv.my401k||"")} save={(v:string)=>upd("my401k",v)}/>
            <F lbl="Your Monthly Contrib." type="number" pre ph="800" def={String(inv.myC||"")} save={(v:string)=>upd("myC",v)}/>
          </div>
          <div style={R2}>
            <F lbl="Spouse 401(k) Balance" type="number" pre ph="40,000" def={String(inv.sp401k||"")} save={(v:string)=>upd("sp401k",v)}/>
            <F lbl="Spouse Monthly Contrib." type="number" pre ph="600" def={String(inv.spC||"")} save={(v:string)=>upd("spC",v)}/>
          </div>
          <div style={R2}>
            <F lbl="Your 457(b) Balance" type="number" pre ph="0" def={String(inv.my457b||"")} save={(v:string)=>upd("my457b",v)}/>
            <F lbl="Spouse 457(b) Balance" type="number" pre ph="0" def={String(inv.sp457b||"")} save={(v:string)=>upd("sp457b",v)}/>
          </div>
          <F lbl="Employer Match %" type="number" ph="50" def={String(inv.match||"")} save={(v:string)=>upd("match",v)} full/>
        </Card>
        <Card>
          <div style={{fontSize:14,fontWeight:700,marginBottom:10}}>Roth IRA</div>
          <F lbl="Combined Monthly Contribution" type="number" pre ph="583" def={String(inv.roth||"")} save={(v:string)=>upd("roth",v)} full/>
          <div style={{fontSize:11,color:"#4a6080"}}>Max $7,000/yr each = $1,167/mo combined</div>
        </Card>
        <div style={ST}>👶 Son's Accounts</div>
        <Card>
          <div style={R2}>
            <F lbl="529 Monthly Contribution" type="number" pre ph="200" def={String(inv.s529||"")} save={(v:string)=>upd("s529",v)}/>
            <F lbl="UTMA Monthly Contribution" type="number" pre ph="100" def={String(inv.utma||"")} save={(v:string)=>upd("utma",v)}/>
          </div>
          <div style={{fontSize:11,color:"#4a6080"}}>529: tax-free college · UTMA: custodial investing (transfers at 18–21)</div>
        </Card>
        <div style={ST}>📊 Retirement Projection</div>
        <Card>
          <div style={R2}>
            <F lbl="Your Age" type="number" ph="35" def={String(retire.myAge||"")} save={(v:string)=>upR("myAge",v)}/>
            <F lbl="Retire At Age" type="number" ph="65" def={String(retire.retireAt)} save={(v:string)=>upR("retireAt",v)}/>
          </div>
          <F lbl="Expected Annual Return %" type="number" ph="7" def={String(retire.rate)} save={(v:string)=>upR("rate",v)} full/>
        </Card>
  ...
  const Goals = () => {
    ...
    const updH=async(field:keyof HysaState,val:string)=>{const nh={...hysa,[field]:parseFloat(val)||0};setHysa(nh);await saveSettings({hysa:nh});};
    const updMF=async(field:keyof FunAcc,val:string)=>{const n={...myFun,[field]:parseFloat(val)||0};setMyFun(n);await saveSettings({myFun:n});};
    const updSF=async(field:keyof FunAcc,val:string)=>{const n={...spFun,[field]:parseFloat(val)||0};setSpFun(n);await saveSettings({spFun:n});};
    const updSc=async(field:keyof Scores,val:string)=>{const n={...scores,[field]:parseFloat(val)||0};setScores(n);await saveSettings({scores:n});};
    return(
      <div style={scr}>
        ...
        <Card>
          ...
          <div style={{...R2,marginTop:12}}>
            <F lbl="Current Balance" type="number" pre ph="0" def={String(hysa.efBal||"")} save={(v:string)=>updH("efBal",v)}/>
            <F lbl="Monthly Contribution" type="number" pre ph="200" def={String(hysa.efC||"")} save={(v:string)=>updH("efC",v)}/>
          </div>
        ...
        <Card>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:12}}>
            <F lbl="Balance" type="number" pre ph="0" def={String(hysa.vacBal||"")} save={(v:string)=>updH("vacBal",v)}/>
            <F lbl="Goal" type="number" pre ph="5,000" def={String(hysa.vacGoal||"")} save={(v:string)=>updH("vacGoal",v)}/>
            <F lbl="Monthly" type="number" pre ph="200" def={String(hysa.vacC||"")} save={(v:string)=>updH("vacC",v)}/>
          </div>
        </Card>
        <div style={ST}>💸 Personal Fun Money</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <Card sx={{margin:0}}>
            <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>👤 Your Account</div>
            <div style={{fontSize:22,fontWeight:800,color:"#2ecc71",marginBottom:8}}>{usd(myFun.bal)}</div>
            <F lbl="Balance" type="number" pre ph="0" def={String(myFun.bal||"")} save={(v:string)=>updMF("bal",v)}/>
            <div style={{marginTop:8}}><F lbl="Monthly" type="number" pre ph="150" def={String(myFun.mo||"")} save={(v:string)=>updMF("mo",v)}/></div>
          </Card>
          <Card sx={{margin:0}}>
            <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>👤 Spouse Account</div>
            <div style={{fontSize:22,fontWeight:800,color:"#2ecc71",marginBottom:8}}>{usd(spFun.bal)}</div>
            <F lbl="Balance" type="number" pre ph="0" def={String(spFun.bal||"")} save={(v:string)=>updSF("bal",v)}/>
            <div style={{marginTop:8}}><F lbl="Monthly" type="number" pre ph="150" def={String(spFun.mo||"")} save={(v:string)=>updSF("mo",v)}/></div>
          </Card>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          {(["me","sp"] as (keyof Scores)[]).map(key=>{
            const val=scores[key]||0;
            const lbl=key==="me"?"Your Score":"Spouse Score";
            return(
              <Card key={key} sx={{margin:0,border:`2px solid ${val?scColor(val):"#263852"}`,textAlign:"center"}}>
                <div style={{fontSize:10,color:"#8fa3be",marginBottom:8}}>{lbl.toUpperCase()}</div>
                <div style={{fontSize:36,fontWeight:800,color:val?scColor(val):"#4a6080"}}>{val||"—"}</div>
                {val>0&&<div style={{fontSize:12,fontWeight:600,color:scColor(val),marginTop:4}}>{scLabel(val)}</div>}
                <div style={{marginTop:10}}><F type="number" ph="720" def={String(val||"")} save={(v:string)=>updSc(key,v)}/></div>
              </Card>
            );
          })}
        </div>
  ...
  const views: Record<string,React.ReactElement> = {
    dash:<Dash/>,inc:<Inc/>,bills:<Bills2/>,exp:<Exp/>,cc:<CC/>,inv:<Inv/>,goals:<Goals/>
  };
  ...
  if (loading) return (
    <div style={{fontFamily:"'SF Pro Display',-apple-system,sans-serif",background:"#0b1624",color:"#e8edf4",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      ...
      {syncErr&&<div style={{fontSize:13,color:"#e74c3c",marginTop:8,textAlign:"center",padding:"0 32px"}}>⚠ Could not connect. Check your API key and Sheet ID.</div>}
    </div>
  );
